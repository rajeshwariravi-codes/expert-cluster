...
if ($Connection) {
    ...
    function Sent_Mail($Seeker_Name, $E_Mail, $Password, $Seeker_ID)
    {
        ...
        $message = '
            ...
            <head>
                <style>
                    ...
                    .header {
                        ...
                        font-size: 24px;
                        font-weight: 700;
                        color: #10b981; /* green = access restored */
                        margin-bottom: 20px;
                        text-align: center;
                    }
            ...
            <body>
                <div class="container">
                    <div class="header">Access Restored 🎉</div>
                    ...
                    <p>
                        Good news—your learner account has been <strong>re-granted access</strong> by the admin team on the 
                        <strong>Expert Cluster</strong> platform. You’re officially back in the loop.
                    </p>

                    <div class="highlight-box">
                        <div class="value">
                            <span class="label">Seeker ID:</span><br>
                            <strong>' . htmlspecialchars($Seeker_ID) . '</strong>
                        </div>

                        <div class="value">
                            <span class="label">New Temporary Password:</span><br>
                            <span class="temp-password">' . htmlspecialchars($Password) . '</span>
                        </div>
                    </div>

                    <p class="note">
                        Use the above password to log in and regain full access to expert streams, queries, and learning tools.
                        For security best practices, we strongly recommend updating your password immediately after logging in.
                    </p>

                    <p class="note">
                        This is your green signal—explore, ask, connect, and keep your learning momentum moving forward.
                    </p>
    ...
    if (isset($_POST['Re_Grant'])) {
        ...
        $Gate = "Re-Grant";
        $Update_Query = "UPDATE `Seeker` SET `Password` = ?, `Access_Gate` = ? WHERE `Seeker_ID` = ?";
        $Update_Query_SQL = $Connection->prepare($Update_Query);
        $Update_Query_SQL->bind_param("sss", $Password, $Gate, $Seeker_ID);
        if ($Update_Query_SQL->execute()) {
            Sent_Mail($Seeker_Name, $E_Mail, $Password, $Seeker_ID);
            echo "
                <script>
                    alert ('Seeker access granted again!✨');
                    window.location.href = 'A_Seeker_Access_Reactivate.php';
                </script>
            ";
        }
    }
...
<main class="content">
    <div class="Wrapper">
        <div class="Nav-Bar container-fluid d-flex flex-wrap justify-content-center align-items-center gap-3 mb-4 p-3">
            <button onclick="window.location.href = 'A_Seeker_Access_Control.php'" class="btn">Back</button>
            <button onclick="window.location.href = 'A_Seeker_Access_Restrict.php'" class="btn">Restrict</button>
            <button onclick="window.location.href = 'A_Seeker_Access_Reactivate.php'" class="btn active">Reactivate</button>
        </div>
        <div class="Header">
                    ...
                    <div class="col-md-4 mb-4 d-flex justify-content-center align-items-center">
                        <div class="card shadow-sm rounded-4 overflow-hidden">
                            ...
                            <div class="card-body d-flex flex-column">
                                ...
                                <div class="card-body">
                                    <h6 class=""><strong>Seeker Type : </strong><?= html_entity_decode($Row['Seeker_Type']) ?></h6>
                                    <h6 class=""><strong>Skilled In : </strong><?= html_entity_decode($Row['Skill']) ?></h6>
                                </div>
                                ...
                                <div class="card-footer d-flex justify-content-center">
                                    <form action="" method="post">
                                        <input type="hidden" name="Seeker_ID" id="Seeker_ID" value="<?= $Row['Seeker_ID'] ?>">
                                        <input type="hidden" name="Seeker_Name" id="Seeker_Name" value="<?= html_entity_decode($Row['First_Name']) . " " . html_entity_decode($Row['Last_Name']) ?>">
                                        <input type="hidden" name="E_Mail" id="E_Mail" value="<?= html_entity_decode($Row['E_Mail']) ?>">
                                        <?php
                                        echo "
                                            <button type='submit' class='btn fs-5' onclick='' name='Re_Grant'>Re-Grant</button>
                                        ";
                                        ?>
                                    </form>
                                </div>
