...
<body>
    ...
    if ($Connection) {
        ...
        function Sent_Code_Mail($V_Expert_Name, $V_E_Mail, $Verification_Code)
        {
            $Name = htmlentities($V_Expert_Name);
        ...
        function Sent_Re_Code_Mail($Re_Expert_Name, $Re_Send_E_Mail, $Verification_Code, $Verification_Code_Timer)
        {
            $Name = htmlentities($Re_Expert_Name);
        ...
        if (isset($_POST['Submit'])) {
            ...
            $Expert_Password = $_POST["Password"];

            $E_Mail_Error = "";
            $Password_Error = "";

            if (!empty($E_Mail_ID) && !empty($Expert_Password)) {
                if (!preg_match('~[\w\.-]+@[\w\.-]+\.\w{2,}~', $E_Mail_ID)) {
                    $E_Mail_Error = "Please enter a valid email address.";
                    $E_Mail_Value = "";
                } elseif (!preg_match('~^(?=.*[A-Z])(?=.*\d)[A-Za-z\d@$!%*?&]{8,}$~', $Expert_Password)) {
                    $Password_Error = "Password must be at least 8 characters, include 1 uppercase letter and 1 number, and a special character (@ $ ! % * ? &).";
                    $E_Mail_Value = $E_Mail_ID;
                } else {
                    $Grant = "Grant";
                    $Re_Grant = "Re-Grant";
                    $Login_Status = "In-Active";
                    $Select_Query = "SELECT * FROM `Expert` WHERE `E_Mail` = ? AND `Password` = ? AND ( `Access_Gate` = ? OR `Access_Gate` = ? ) AND ( `Login_Status` = ? OR `Login_Status` IS NULL )";
                    $Select_Query_SQL = $Connection->prepare($Select_Query);
                    $Select_Query_SQL->bind_param("sssss", $E_Mail_ID, $Expert_Password, $Grant, $Re_Grant, $Login_Status);
                    $Select_Query_SQL->execute();
                    $Result = $Select_Query_SQL->get_result();

                    if ($Row = $Result->fetch_assoc()) {
                        $E_Expert_ID    = $Row['Expert_ID'];
                        $E_E_Mail_Id    = $Row['E_Mail'];
                        $E_Password     = $Row['Password'];
                    } else {
                        echo "
                        <script>
                            alert('Verify your login details — only granted or re-granted Experts can access this portal.');
                            window.location.href = 'Expert_Sign_In.php';
                        </script>
                    ";
                    }
                }
            ...
            if (!empty($E_E_Mail_Id) && !empty($E_Password)) {
                if ($E_E_Mail_Id == $E_Mail_ID && $E_Password == $Expert_Password) {
                    $Login_Status = "Active";
                    $Update_Status_Query = "UPDATE `Expert` SET `Login_Status` = ? WHERE `Expert_ID` = ?";
                    $Update_Status_Query_SQL = $Connection->prepare($Update_Status_Query);
                    $Update_Status_Query_SQL->bind_param("ss", $Login_Status, $E_Expert_ID);

                    if ($Update_Status_Query_SQL->execute()) {
                        $_SESSION['Expert_ID'] = $E_Expert_ID;
                        echo "
                        <script>
                            alert('Login Successfull ✨');
                            window.location.href = 'Expert_Dashboard.php';
                        </script>
                    ";
                    }
                }
        ...
        if (isset($_POST['Sent'])) {
            $V_Expert_Name = $_POST["Expert_Name"];
            ...
            $Grant = "Grant";
            $Re_Grant = "Re-Grant";
            $Verification_Code_Query = "UPDATE `Expert` SET `Verification_Code` = ? , `Verification_Code_Timer` = ? WHERE E_Mail = ? AND ( Access_Gate = ? OR Access_Gate = ? )";
            $Verification_Code_Query_SQL = $Connection->prepare($Verification_Code_Query);
            $Verification_Code_Query_SQL->bind_param("sssss", $Verification_Code, $Verification_Code_Timer, $V_E_Mail, $Grant, $Re_Grant);
            if ($Verification_Code_Query_SQL->execute()) {
                if ($Verification_Code_Query_SQL->affected_rows > 0) {
                    Sent_Code_Mail($V_Expert_Name, $V_E_Mail, $Verification_Code);
                    echo "
                    <script>
                ...
                ";
                    ...
                    echo "
                    <script>
                        alert('Verify your details — only granted or re-granted Experts can access this portal.');
                        window.location.href = 'Expert_Sign_In.php';
                    </script>
                ";
                }
        ...
        if (isset($_POST['New_Pass'])) {
            ...
            $Select_Name_Query = "SELECT * FROM `Expert` WHERE E_Mail = ?";
            ...
            if ($Row = $Result->fetch_assoc()) {
                ...
                // $V_Expert_Name = $Cleared['Expert_Name'];
                ...
                if (!empty($Verification_Code_Timer) && $Current_Time > strtotime($Verification_Code_Timer)) {
                    ...
                    if ($Cleared['N_Password'] === $Cleared['Re_N_Password']) {
                        ...
                        $Update_Query = "UPDATE `Expert` SET Password = ?, Verification_Code = '', Verification_Code_Timer = '' WHERE E_Mail = ? AND Verification_Code = ?";
        ...
        if (isset($_POST['Re_Send'])) {
            $Re_Expert_Name = $_POST['Expert_Name'];
            ...
            $Verification_Code_Query = "UPDATE `Expert` SET `Verification_Code` = ? , `Verification_Code_Timer` = ? WHERE E_Mail = ?";
            $Verification_Code_Query_SQL = $Connection->prepare($Verification_Code_Query);
            ...
            if ($Verification_Code_Query_SQL->execute()) {
                ...
                Sent_Re_Code_Mail($Re_Expert_Name, $Re_Send_E_Mail, $Verification_Code, $Verification_Code_Timer);
                echo "
                    <script>
                        alert ('Verification Code Sent Successfully✨');
                        var myModal = new bootstrap.Modal(document.getElementById('exampleModalToggle2'));
    ...
    <div class="Wrapper container justify-content-center">
        <div class="w-100 w-md-auto p-3">
            <div class="Expert-Details">
                <h3 class="Title text-center">Expert Sign In</h3>
                    ...
                    <div class="Form w-100">
                        <div class="form-group md-3">
                            ...
                            <div class="position-relative">
                                ...
                                <input class="form-control" type="email" name="Mail_Id" id="Mail_Id" value="<?= $E_Mail_Value ?>" data-next="Password" placeholder="Enter The E-Mail Id" autofocus required autocomplete="off">
                            </div>
                        </div>
                        ...
                        <div class="Already text-center mt-3">
                            <p class="small mb-0">
                                ...
                                <a href="./Expert_Sign_Up.php">Sign Up</a>
                            </p>
                        </div>
                    </div>
                </form>
                ...
                <form method="post">
                    <div class="modal-body">
                        <div class="form-group mb-3">
                            <label for="Expert_Name" class="col-form-label">Full Name :</label>
                            <input type="text" class="form-control Modal_Input" name="Expert_Name" id="Expert_Name" placeholder="Enter Your Full Name." required autocomplete="off">
                        </div>
                        <div class="form-group mb-3">
    ...
    <div class="modal fade" id="exampleModalToggle2" data-bs-backdrop="static" data-bs-heyboard="false" aria-hidden="true" aria-labelledby="exampleModalToggleLabel2" tabindex="-1">
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <form method="post">
                    <div class="modal-body">
                        <input class="form-control Modal_Input" type="hidden" name="Expert_Name" id="Expert_Name" value="<?= $V_Expert_Name ?>" required readonly>
                        <div class="form-group mb-3">
                            <label for="E_Mail" class="col-form-label">E-Mail :</label>
