...
if ($Connection) {
    ...
    function Sent_Mail($Expert_Name, $E_Mail, $Expert_ID)
    {
        ...
        $message = '
                ...
                <body>
                    <div class="container">
                        <div class="header">Welcome to Expert Cluster, ' . $Name . '!</div>

                        <p>
                            Congratulations! Your expert profile has been <strong>successfully granted</strong> by the admin. 
                            You can now log in to the <strong>Expert Cluster</strong> platform and start teaching learners.
                        </p>
    ...
    if (isset($_POST['Grant'])) {
        ...
        $Gate = "Grant";
        $Update_Query = "UPDATE `Expert` SET `Access_Gate` = ? WHERE `Expert_ID` = ?";
        $Update_Query_SQL = $Connection->prepare($Update_Query);
        $Update_Query_SQL->bind_param("ss", $Gate, $Expert_ID);
        if ($Update_Query_SQL->execute()) {
            Sent_Mail($Expert_Name, $E_Mail, $Expert_ID);
            echo "
                <script>
                    alert ('Expert granted successfully✨');
...
<main class="content">
    <div class="Wrapper">
        <div class="Nav-Bar container-fluid d-flex flex-wrap justify-content-center align-items-center gap-3 mb-4 p-3">
            ...
            <button onclick="window.location.href = 'A_Expert_Access_Activate.php'" class="btn active">Activate</button>
                            ...
                            <div class="card-body d-flex flex-column mt-4">
                                ...
                                <div class="card-footer d-flex justify-content-center">
                                    <form action="" method="post">
                                        ...
                                        <?php
                                        echo "
                                            <button type='submit' class='btn fs-5' name='Grant'>Grant</button>
                                        ";
                                        ?>
                                    </form>
                                </div>
