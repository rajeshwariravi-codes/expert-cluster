...
if ($Connection) {
    ...
    if (isset($_POST['Ask'])) {
        ...
        if (empty($Stream)) {
            echo "
                <script>
                    alert ('Stream missing 🚀 Pick one to continue.');
                    window.location.href = 'S_Experts_Query.php';
                </script>
            ";
        } else {
            $_SESSION['Expert_ID'] = $Expert_ID;
            $_SESSION['Stream_ID'] = $Stream_ID;

            echo "
                <script>
                    alert ('Choice confirmed ✨ Drop your question.');
                    window.location.href = 'S_Expert_Query_Form.php';
                </script>
            ";
        }
    }
...
<link rel="stylesheet" href="./S_Experts.css">

<main class="content">
    <div class="Wrapper">
        <div class="Nav-Bar container-fluid d-flex flex-wrap justify-content-center align-items-center gap-3 mb-4 p-3">
            <button onclick="window.location.href = 'S_Query_Control.php'" class="btn">Back</button>
            <button onclick="window.location.href = 'S_Streams_Query.php'" class="btn">Streams</button>
            <button onclick="window.location.href = 'S_Experts_Query.php'" class="btn active">Experts</button>
        </div>
        <div class="Header">
                        ...
                        <div class="card shadow-sm rounded-4 overflow-hidden">
                            ...
                            <div class="card-body d-flex flex-column mt-4">
                                ...
                                <div class="card-body">
                                    ...
                                    <h6 class=""><strong>Organization : </strong><?= html_entity_decode($Row['Organization_Name']) ?><strong class="m-2 p-1 badge" style="background: rgba(103, 48, 127, 0.8); font-family: 'Aboreto';"><?= html_entity_decode($Row['Years_of_Experience']) ?></strong></h6>
                                    <h6 class="card-text"><strong>Streams : </strong></h6>
                                    <?php
                                    $Stream = array_map('trim', explode(',', $Row['Streams']));
                                    foreach ($Stream as $Subject) {
                                    ?>
                                        <button class="Stream_Btn p-1" onclick="applyStreamFilter('<?= $Subject ?>')">
                                        ...
                                        <div class="d-flex justify-content-between gap-2">
                                            ...
                                            <div class="form-group">
                                                <select class="form-select" name="Stream_ID" id="Stream_ID">
                                                    <option value="" selected disabled hidden>-- Select Steream --</option>
                                                    <?php
                                                    if ($Connection) {
                                                        $Grant = "Grant";
                                                        $Re_Grant = "Re-Grant";
                                                        $Select__Stream_Query = "SELECT `Stream_Name`, `Stream_ID` FROM `Expert_Streams` WHERE `Expert_ID` = ? AND `Stream_Name` IN (SELECT `Stream` FROM `Streams` WHERE (`Access_Gate` = ? OR `Access_Gate` = ?))";
                                                        $Select__Stream_Query_SQL = $Connection->prepare($Select__Stream_Query);
                                                        $Select__Stream_Query_SQL->bind_param("sss", $Row['Expert_ID'], $Grant, $Re_Grant);
                                                        $Select__Stream_Query_SQL->execute();
                                                        $Stream_Result = $Select__Stream_Query_SQL->get_result();
                                                        while ($Stream_Row = $Stream_Result->fetch_assoc()) {
                                                    ?>
                                                            <option value="<?= $Stream_Row['Stream_ID'] ?>"><?= html_entity_decode($Stream_Row['Stream_Name']) ?></option>
                                            ...
                                            <button type="submit" name="Ask" id="Ask" class="btn fs-6">Ask</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
